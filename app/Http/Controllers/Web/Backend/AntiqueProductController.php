...
class AntiqueProductController extends Controller
{
    public $part;
    public $route;
    public $view;
    ...
    {
        if ($request->ajax()) {
            $data = AntiqueProduct::orderBy('position', 'asc')->orderBy('id', 'desc')->get();
        ...
        $cms = CMS::where('page', 'antique_collection')->where('section', 'hero')->first();
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $item = new AntiqueProduct();
            $item->title = $data['title'];
            $item->slug = Helper::makeSlug(AntiqueProduct::class, $data['title']);
            ...
            if ($request->hasFile('image')) {
                $item->thumbnail = Helper::fileUpload($request->file('image'), 'antique_product', time() . '_' . getFileName($request->file('image')));
            }
    ...
    {
        $item = AntiqueProduct::findOrFail($id);
    ...
    {
        try {
            $data = AntiqueProduct::findOrFail($id);
    ...
    public function updateCms(Request $request)
    {
        ...
        try {
            $section = CMS::where('page', 'antique_collection')->where('section', 'hero')->first();

            $data = [
                'page'      => 'antique_collection',
                'section'   => 'hero',
                'title'     => $request->title,
            ...
            if ($request->hasFile('image')) {
                ...
                $data['image'] = Helper::fileUpload($request->file('image'), 'antique_collection', time() . '_' . getFileName($request->file('image')));
            }
