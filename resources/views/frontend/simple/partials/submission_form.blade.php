...
<div class="card border-0 modern-glass-card rounded-32 p-4 p-md-5 shadow-premium">
    ...
    @if(session('t-success') || session('success'))
        <div class="alert alert-success p-4 rounded-24 border-0 shadow-premium mb-4" role="alert"
            style="background-color: #d1fae5; border: 1px solid #a7f3d0 !important;">
            <div class="d-flex align-items-center mb-3">
                <div class="bg-success text-white rounded-circle d-flex align-items-center justify-content-center me-3"
                    style="width: 48px; height: 48px; flex-shrink: 0; background-color: #10b981 !important;">
                    <i class="fe fe-check fs-4"></i>
                </div>
                <div class="text-start">
                    <h4 class="fw-bold text-emerald-900 mb-0" style="color: #065f46;">Thank You!</h4>
                    <p class="text-emerald-800 small mb-0" style="color: #047857;">
                        {{ session('t-success') ?? session('success') }}
                    </p>
                </div>
            </div>
    ...
    @if(session('t-error') || session('error'))
        <div class="alert alert-danger alert-dismissible fade show rounded-16 p-3 mb-4" role="alert"
        ...
        <div class="row g-3">
            ...
            @if(isset($items) && $items->isNotEmpty())
                ...
                <div class="col-12 text-start">
                    <label class="form-label text-slate-700 fw-semibold small"><i
                            class="fe fe-layers me-1 text-primary"></i> Select Module</label>
            ...
            @elseif(isset($item))
                ...
                @if($type === 'course')
                    ...
                    <div class="col-12 text-start">
                        <label class="form-label text-slate-700 fw-semibold small"><i
                                class="fe fe-layers me-1 text-primary"></i> Enrolling In</label>
                ...
                @elseif($type === 'product')
                    ...
                    <div class="col-12 text-start">
                        <label class="form-label text-slate-700 fw-semibold small"><i
                                class="fe fe-shopping-cart me-1 text-primary"></i> Selected Product</label>
                        <input type="text" class="form-control custom-input" value="{{ $item->title }}" disabled>
                    </div>
                @elseif($type === 'service')
                    ...
                    <div class="col-12 text-start">
                        <label class="form-label text-slate-700 fw-semibold small"><i
                                class="fe fe-briefcase me-1 text-primary"></i> Requested Service</label>
                        <input type="text" class="form-control custom-input" value="{{ $item->title }}" disabled>
                    </div>
                @endif
            @endif
            ...
            {{-- Core form fields --}}
            <div class="col-12 text-start">
                <label class="form-label text-slate-700 fw-semibold small"><i class="fe fe-user me-1 text-primary"></i>
                    Your Name</label>
            ...
            {{-- <div class="col-12 text-start">
                <label class="form-label text-slate-700 fw-semibold small"><i
                        class="fe fe-map-pin me-1 text-primary"></i> Address</label>
                <input type="text" name="address" class="form-control custom-input" placeholder="Your Address"
                    value="{{ old('address') }}">
            </div> --}}
            <div class="col-md-6 text-start">
                <label class="form-label text-slate-700 fw-semibold small"><i class="fe fe-phone me-1 text-primary"></i>
                    Payment Number</label>
                <input type="text" name="phone" class="form-control custom-input" placeholder="01XXXXXXXXX"
                    value="{{ old('phone') }}" required>
            </div>
            @if($type !== 'service')
                <div class="col-md-6 text-start">
                    <label class="form-label text-slate-700 fw-semibold small"><i class="fe fe-mail me-1 text-primary"></i>
                        Email</label>
                    <input type="email" name="email" class="form-control custom-input" placeholder="email@example.com"
            ...
            @if($type !== 'service')
                ...
                @if($bkashNumber || $nagadNumber)
                    <div class="col-12 mt-4 text-start">
                        <label class="form-label text-slate-700 fw-semibold small d-block mb-2"><i
                                class="fe fe-credit-card me-1 text-primary"></i> Select Payment Method & Pay</label>
                        <div class="d-flex gap-3 mb-3">
                            @if($bkashNumber)
                                <div class="payment-method-card flex-fill text-center p-3 rounded-20 border cursor-pointer d-flex flex-column align-items-center justify-content-center"
                                    id="method-bkash" onclick="selectPayment('bkash', '{{ $bkashNumber }}')">
                                    <img src="{{ asset('default/bkash.svg') }}" alt="bKash"
                                        class="payment-logo mb-2 animate__animated animate__fadeIn"
                            ...
                            @if($nagadNumber)
                                <div class="payment-method-card flex-fill text-center p-3 rounded-20 border cursor-pointer d-flex flex-column align-items-center justify-content-center"
                                    id="method-nagad" onclick="selectPayment('nagad', '{{ $nagadNumber }}')">
                                    <img src="{{ asset('default/nagad.svg') }}" alt="Nagad"
                                        class="payment-logo mb-2 animate__animated animate__fadeIn"
                                        style="height: 38px; object-fit: contain; margin-top: -3px;">
                        ...
                        <div id="payment-instruction-box"
                            class="bg-light p-3 rounded-20 border mb-3 d-none animate__animated animate__fadeIn">
                            ...
                            <div class="d-flex align-items-center justify-content-between bg-white p-2 px-3 rounded-16 border">
                                ...
                                <button type="button" class="btn btn-sm btn-outline-secondary rounded-pill px-3 py-1 fs-12"
                                    id="copy-btn" onclick="copyNumber()">Copy</button>
                            </div>
                        ...
                        <div id="transaction-id-group" class="form-group d-none animate__animated animate__fadeIn">
                            <label for="transaction_id" class="form-label text-slate-700 fw-semibold small">Transaction ID
                                (TrxID)</label>
                            <input type="text" name="transaction_id" id="transaction_id" class="form-control custom-input"
                                placeholder="e.g. 8N70XDPQ9S" value="{{ old('transaction_id') }}">
                        </div>
                    </div>
                @endif
            @endif
            ...
            <div class="col-12 mt-4">
                <button type="submit"
                    class="btn btn-gradient-primary w-100 rounded-pill py-3 fw-bold text-white shadow-premium hover-scale">
