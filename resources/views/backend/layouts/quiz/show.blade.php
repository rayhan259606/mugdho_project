@extends('backend.app', ['title' => 'Show Quiz'])
                    ...
                    <div class="card post-sales-main">
                        ...
                        <div class="card-body">
                            <table class="table table-bordered table-striped">
                                    ...
                                    <td>
                                        <ul>
                                            @foreach (json_decode($quiz->options) as $option)
                                                <li style="border-bottom: 1px solid black; padding: 5px">{{ $option }}</li>
                                            @endforeach
                                        </ul>
                                    </td>
                                </tr>
                                <tr>
                                    <th>Answer</th>
                                    <td>{!! $quiz->answer ?? 'N/A' !!}</td>
                                </tr>
                                <tr>
                                    <th>Action</th>
                                    <td>
                                        <button class="btn btn-sm btn-danger" onclick="showDeleteConfirm(`{{ $quiz->id }}`)">Delete</button>
                                        <button class="btn btn-sm btn-primary" onclick="goToEdit(`{{ $quiz->id }}`)">Edit</button>
                                    </td>
                                </tr>
                            </table>
                        </div>
